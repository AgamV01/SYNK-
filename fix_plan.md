...
- [x] 114. main.ts wire SDK + scene + ui; connect to server. verify: `cd client && npm run build`
...
- [x] 120. Final integration gate: security scan clean, then server cov + client tsc + build all green. verify: run the security scan, then `cd server && pytest --cov=synk --cov-fail-under=80 && cd ../client && npx tsc --noEmit && npm run build`
